...
module.exports = function(RED) {
    ...
    function findVariables(str) {
        ...
        //clean %
        for (var i in found) {
            found[i] = found[i].substr(1,found[i].length-2);
        }

        return found;
    }

    function resolveVariables(url) {
        var envs = findVariables(url);
        ...
        for (var env in envs) {
            ...
            util.log('[mqttenv] Resolving '+env+' to '+process.env[env]);
            url = url.replace('%'+env+'%', process.env[env]);
        }
        return url;
    }
    ...
    function MQTTBrokerNode(n) {
        ...
        var b = resolveVariables(this.broker);
        if (!b) {
            this.error('Failed to resolve a env variable.');
            return;
        } else {
            this.log('Broker was resolved to '+b);
        }
        this.broker = b;
        ...
        this.options.clientId = this.clientid || 'mqttenv_' + (1+Math.random()*4294967295).toString(16);
    ...
    RED.nodes.registerType("mqttenv-broker",MQTTBrokerNode,{
        credentials: {
            user: {type:"text"},
    ...
    RED.nodes.registerType("mqttenv in",MQTTInNode);
    ...
    RED.nodes.registerType("mqttenv out",MQTTOutNode);
};
